...
_sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_15 = Integer(15); _sage_const_2 = Integer(2); _sage_const_4 = Integer(4); _sage_const_6 = Integer(6); _sage_const_8 = Integer(8); _sage_const_3 = Integer(3); _sage_const_5 = Integer(5); _sage_const_7 = Integer(7); _sage_const_9 = Integer(9); _sage_const_1000 = Integer(1000)# L = meet-semidistributive lattice
def JoinKappa(L, j):
    # we suppose that L is meet-semidistributive
    assert j in L.join_irreducibles()
    js = L.lower_covers(j)[_sage_const_0 ]
    while true:
        if L.is_lequal(j, L.upper_covers(js)[_sage_const_0 ]):
            if len(L.upper_covers(js)) == _sage_const_1 :
                return js
            js = L.upper_covers(js)[_sage_const_1 ]
        else:
            js = L.upper_covers(js)[_sage_const_0 ]
    ...
    for x in MOPLattice(G, False):
        for s1 in Subsets(Set(x)):
            for s2 in Subsets(Set(right_orthogonal(G, x))):
                S.add((s1,  s2))
    if lattice:
        return LatticePoset((S, lambda p, q: p[_sage_const_0 ].issubset(q[_sage_const_0 ]) and q[_sage_const_1 ].issubset(p[_sage_const_1 ])))
    return S
...
def DrawTAFS(G, size=_sage_const_15 ):
    S = SurEdges(G)
    I = InEdges(G)
    colI, colS, colIS, col = 'red','blue','black','green'
    D = {colI:[], colS:[], colIS:[], col:[]}
    for e in G.edges():
        if e[:_sage_const_2 ] in S:
            if e[:_sage_const_2 ] in I:
                D[colIS].append(e)
            else:
                D[colS].append(e)
        else:
...
def ForcingOrder(G):
    ...
    C = {c[_sage_const_0 ]: c for c in D.strongly_connected_components()}
    for x in C:
...
def WeakOrderJoinRepr(W,w):
    ...
    R = []
    for s in w.descents():
        r = w*S[s-_sage_const_1 ]*w.inverse()
        w2 = w
        while True:
            t = False
            for s2 in w2.descents():
                if w2*S[s2-_sage_const_1 ]*w2.inverse()!=r:
                    w2 = w2*S[s2-_sage_const_1 ]
                    t = True
                    break
            if not t:
                R.append(w2)
...
def LatticeFaces(L,lattice = True):
    ...
    if lattice:
        return Poset((LF,lambda p,q:L.is_lequal(p[_sage_const_0 ],q[_sage_const_0 ]) and L.is_lequal(p[_sage_const_1 ],q[_sage_const_1 ])))
    return LF

def FaceJoin(L,f1,f2):
    x,y = L.join(f1[_sage_const_0 ],f2[_sage_const_0 ]),L.join(f1[_sage_const_1 ],f2[_sage_const_1 ])
    l = [y]
...
def LatticeFaces2(L):
    ...
    G2 = Derived_Graph(G,[_sage_const_0 ,_sage_const_2 ,_sage_const_4 ,_sage_const_6 ,_sage_const_8 ])
    L2 = MOPLattice(G2)
    def face_relabel(X):
        Y = right_orthogonal(G2,X)
        return Set([x for x,k in X if k==_sage_const_0 ]),Set(left_orthogonal(G,[x for x,k in Y if k==_sage_const_1 ]))
...
def SD_ASMs(n):
    P = Poset((Triples(n),Ordre(Poset({_sage_const_0 :[_sage_const_2 ,_sage_const_3 ],_sage_const_1 :[_sage_const_2 ,_sage_const_3 ]}))))
    D = P.hasse_diagram().to_dictionary()
    for x,y in P.intervals_poset():
        if x!=y and not P.covers(x,y):
            if y[_sage_const_0 ]-x[_sage_const_0 ]==y[_sage_const_1 ]-x[_sage_const_1 ] or y[_sage_const_2 ]-x[_sage_const_2 ]==_sage_const_0 :
                D[x].append(y)
    G = DiGraph(D)
    for x,y,_ in G.edges():
        if y[_sage_const_0 ]-x[_sage_const_0 ]==y[_sage_const_1 ]-x[_sage_const_1 ]:
            for z in G.neighbors_in(x):
                if y not in D[z]:
                    D[z].append(y)
        if y[_sage_const_2 ]-x[_sage_const_2 ]==_sage_const_0 :
            for z in G.neighbors_out(y):
                if z not in D[x]:
                    D[x].append(z)
    return DiGraph(D)     
...
def Derived_Graph(G,l):
    ...
    for x in G:
        D[(x,_sage_const_0 )] = []
        D[(x,_sage_const_1 )] = []
        if _sage_const_0  in l:
            D[(x,_sage_const_0 )].append((x,_sage_const_1 ))
        if _sage_const_1  in l:
            D[(x,_sage_const_1 )].append((x,_sage_const_0 ))
        for y in G.neighbors_out(x):
            if x!=y:
                if _sage_const_2  in l:
                    D[(x,_sage_const_0 )].append((y,_sage_const_0 ))
                if _sage_const_4  in l:
                    D[(x,_sage_const_0 )].append((y,_sage_const_1 ))
                if _sage_const_6  in l:
                    D[(x,_sage_const_1 )].append((y,_sage_const_0 ))
                if _sage_const_8  in l:
                    D[(x,_sage_const_1 )].append((y,_sage_const_1 ))
        for y in G.neighbors_in(x):
            if x!=y:
                if _sage_const_3  in l:
                    D[(x,_sage_const_0 )].append((y,_sage_const_0 ))
                if _sage_const_5  in l:
                    D[(x,_sage_const_0 )].append((y,_sage_const_1 ))
                if _sage_const_7  in l:
                    D[(x,_sage_const_1 )].append((y,_sage_const_0 ))
                if _sage_const_9  in l:
                    D[(x,_sage_const_1 )].append((y,_sage_const_1 ))
    return DiGraph(D)
...
def TamariIrreducibles(n):
    ...
    for k in range(n-_sage_const_1 ):
        for i in range(_sage_const_1 ,n-k):
            l = n*[_sage_const_0 ]
            l[k] = i
            I.append(tuple(l))
    return I
...
def TamariJoinRepr(v):
    ...
    for i,k in enumerate(reversed(v)):
        if k:
            if l[i-k]:
                J.append(tuple((n-i-_sage_const_1 )*[_sage_const_0 ]+[k-l[i-k]]+i*[_sage_const_0 ]))
            else:
                J.append(tuple((n-i-_sage_const_1 )*[_sage_const_0 ]+[k]+i*[_sage_const_0 ]))
            l[i-k] = k
    return J
...
def TamariMeetRepr(v):
    ...
    for i,k in enumerate(v):
        if k!=n-i-_sage_const_1  and not l[i+k]:
            M.append(tuple(list(range(n-_sage_const_1 ,n-i-_sage_const_1 ,-_sage_const_1 ))+list(range(k,-_sage_const_1 ,-_sage_const_1 ))+list(range(n-i-k-_sage_const_2 ,-_sage_const_1 ,-_sage_const_1 ))))
            l[i+k] = True
    return M

def TamariKMap(v):
    n = len(v)
    K = list(range(n-_sage_const_1 ,-_sage_const_1 ,-_sage_const_1 ))
    for J in TamariJoinRepr(v):
        M = list(range(n-_sage_const_1 ,-_sage_const_1 ,-_sage_const_1 ))
        for i in range(n):
            if J[i]:
...
def TamariCliques(n):
    ...
    return Graph([(C[i],C[j]) for i in range(len(C)) for j in range(i+_sage_const_1 ,len(C)) if len(C[i].intersection(C[j]))==n-_sage_const_2 ])
...
def SimplicesGraph(G,n):
    G.remove_loops()
    Cl = [Set(c) for c in sage.graphs.cliquer.all_max_clique(G)]
    return Graph([(Cl[i],Cl[j]) for i in range(len(Cl)) for j in range(i+_sage_const_1 ,len(Cl)) if len(Cl[i].intersection(Cl[j]))==n-_sage_const_1 ])

#G = SDLGraph(L)
def CliqueSizes(G):
    G.remove_loops()
    Stats(sage.graphs.cliquer.all_cliques(G,_sage_const_1 ),[lambda c:len(c)])


######

def GraphSkewIntervals(L,lattice = True,l = [_sage_const_2 ,_sage_const_4 ,_sage_const_8 ]):
    G = SDLGraph(L).canonical_label()
    ...
    for x in L2:
        ...
        LL.append((Set(left_orthogonal(G,[a[_sage_const_0 ] for a in x2 if a[_sage_const_1 ]==_sage_const_0 ])),Set([a[_sage_const_0 ] for a in x if a[_sage_const_1 ]==_sage_const_1 ])))
    if lattice:
        return LatticePoset((LL,lambda p,q:p[_sage_const_0 ].issubset(q[_sage_const_0 ]) and p[_sage_const_1 ].issubset(q[_sage_const_1 ])))
    return LL

def SkewIntervalsInf(L):
    D = {}
    for s in GraphSkewIntervals(L,False):
        if s[_sage_const_0 ] in D:
            D[s[_sage_const_0 ]] += _sage_const_1 
        else:
            D[s[_sage_const_0 ]] = _sage_const_1 
    return D

def SkewIntervalsPivot(L):
    lp = len(L)*[_sage_const_0 ]
    D = {x:i for i,x in enumerate(L)}
    for s in GraphSkewIntervals(L,False):
        lp[D[s[_sage_const_0 ].intersection(s[_sage_const_1 ])]] += _sage_const_1 
    return lp
...
def SkewIntervalsTest(L,lattice = True):
    ...
    if lattice:
        return LatticePoset((LL,lambda p,q:L.is_lequal(p[_sage_const_0 ],q[_sage_const_0 ]) and L.is_lequal(p[_sage_const_1 ],q[_sage_const_1 ])))
    return LL

def Exponent(L):
    K,C = JoinKMap(L),set(L.coatoms())
    return sum(_sage_const_1  for c in L.atoms() if K[c] in C)
...
def SkewIntervals(L,lattice = True):
    ...
    for x,y in L.intervals_poset():
        ...
        for S in Subsets(cc):
            SI.append((I.join([s[_sage_const_0 ] for s in S]),I.meet([s[_sage_const_1 ] for s in S])))
    if lattice:
        return LatticePoset((SI,lambda p,q:L.is_lequal(p[_sage_const_0 ],q[_sage_const_0 ]) and L.is_lequal(p[_sage_const_1 ],q[_sage_const_1 ])))
    return SI
...
def SkewFaces(L,lattice = True):
    ...
    for x in L:
        for J in Subsets(L.upper_covers(x)):
            ...
            cc = [(c,K[c]) for c in I.atoms() if K[c] in C]
            for S in Subsets(cc):
                SF.append((I.join([s[_sage_const_0 ] for s in S]),I.meet([s[_sage_const_1 ] for s in S])))
    if lattice:
        return LatticePoset((SF,lambda p,q:L.is_lequal(p[_sage_const_0 ],q[_sage_const_0 ]) and L.is_lequal(p[_sage_const_1 ],q[_sage_const_1 ])))
    return SF

#pas intéressant
def NilpotentIntervals(L):
    LI = L.intervals_poset()
    D = {I:_sage_const_0  for I in LI}
    for x,y in LI:
        I = LatticePoset(L.subposet(L.interval(x,y)))
        K,C = JoinKMap(I),set(I.coatoms())
        cc = [c for c in I.atoms() if K[c] in C]
        D[(I.join(cc),y)] += (-_sage_const_1 )**len(cc)
    return D
...
def DPolyIntervals(LI):
    ...
    LI = SkewIntervals(L)
    """
    R4 = ZZ['x,y,z,w']; (x, y, z, w,) = R4._first_ngens(4)
    D = R4(_sage_const_0 )
    for i in LI:
        m = _sage_const_1 
        for j in LI.upper_covers(i):
            if i[_sage_const_0 ]==j[_sage_const_0 ]:
                m *= y
            elif i[_sage_const_1 ]==j[_sage_const_1 ]:
                m *= x
            else:
                print('erreur1')
...
def TamariExponent(v1,v2):
    ...
    for i,k in enumerate(v1):
        if k!=n-i-_sage_const_1  and l1[i+k]!='f':
            l1[i] = k+_sage_const_1 
            if k>_sage_const_0 :
                l1[i+k] = 'f'
    l2 = list(v2)
    for i,k in enumerate(reversed(v2)):
        if k:
            l2[n-i-_sage_const_1 ] = k-l2[n-i-_sage_const_1 +k]
            l2[n-i-_sage_const_1 +k] = k
    return sum(_sage_const_1  for i in range(n) if l1[i]==l2[i] and v2[i]!=_sage_const_0 )

def possible_values(bv):
    n = len(bv)
    L,i = [_sage_const_0 ],_sage_const_0 
    while i < n:
        L.append(bv[i]+i+_sage_const_1 )
        i += bv[i]+_sage_const_1 
    return L 
def TamariIntervals(n):
    L=[]
    if n == _sage_const_1 :
        return([([_sage_const_0 ],[_sage_const_0 ])])
    for bvs in TamariIntervals(n-_sage_const_1 ):
        for b in possible_values(bvs[_sage_const_1 ]):
            for a in possible_values(bvs[_sage_const_0 ]):
                if b>=a:
                    L.append(([a]+bvs[_sage_const_0 ],[b]+bvs[_sage_const_1 ]))
    return L
...
def right_orthogonal2(M,v):
    return vector([int(k==_sage_const_0 ) for k in M*v],immutable = True)
#v encods a subset of Y
def left_orthogonal2(M,v):
    return vector([int(k==_sage_const_0 ) for k in v*M],immutable = True)

def Lattice(M,lattice = True):
    n = len(M[_sage_const_0 ])
    LS = [vector(n*[_sage_const_0 ],immutable = True)]
    L = LS
    while LS!=[]:
        L2 = []
        for s in LS:
            for x in range(n):
                if s[x]==_sage_const_0 :
                    s2 = list(s)
                    s2[x] = _sage_const_1 
...
def RandomOrientation(G):
    ...
    for e in E:
        if randint(_sage_const_0 ,_sage_const_1 ):
            ...
            E2.append((e[_sage_const_1 ],e[_sage_const_0 ]))
    return DiGraph(E2)

#E = liste d'arêtes
def RandomOrientations(E,n = _sage_const_1000 ,LL = set()):
    for k in range(n):
        E2 = []
        for e in E:
            if randint(_sage_const_0 ,_sage_const_1 ):
...
def Slicing_to_chains(G,S):
    ...
    Edges = set()
    for x in L:
        s1,t1 = Set(k for k in x if k in S[_sage_const_0 ]),Set(k for k in right_orthogonal(G,x) if k in S[_sage_const_0 ])
        for i in range(_sage_const_1 ,len(S)):
            s2,t2 = Set(k for k in x if k in S[i]),Set(k for k in right_orthogonal(G,x) if k in S[i])
            Edges.add(((s1,t1,i),(s2,t2,i+_sage_const_1 )))
            s1,t1 = s2,t2
    return DiGraph(list(Edges))

def RandomSlicing(G, x0 = None):
    F = Factorization(G)[_sage_const_0 ]
    if x0 == None:
        x0 = F.random_vertex()
    l = [x0]
    L = [l]
    while len(l)>_sage_const_0 :
        ...
        for x in l:
            for y in F.neighbors(x):
                if y not in L[-_sage_const_1 ] and (len(L)==_sage_const_1  or y not in L[-_sage_const_2 ]):
                    l2.add(y)
        l = list(l2)
        L.append(l)
    return L[:-_sage_const_1 ]

def Tamari_rank_slicing(n):
    G,_ = Factorization(TamariGraph(n))
    f = Poset(G).rank_function()
    S = [[] for _ in range(_sage_const_2 *n-_sage_const_3 )]
    for x in G:
        S[f(x)].append(x)
    return S
...
def TamariSlicingPoset(n):
    ...
    lranks,f = [[] for _ in range(_sage_const_2 *n-_sage_const_3 )],P.rank_function()
    for x in P:
        lranks[f(x)].append(x)
    relabel = {}
    for k in range(_sage_const_2 *n-_sage_const_3 ):
        for x in lranks[k]:
            t = min(k//_sage_const_2 +_sage_const_1 ,n-(k+_sage_const_1 )//_sage_const_2 -_sage_const_1 )*[_sage_const_0 ]
            for y in x[_sage_const_0 ]:
                t[(sum(y)-_sage_const_1 )//_sage_const_2 ] = -_sage_const_1 
            for y in x[_sage_const_1 ]:
                t[(sum(y)-_sage_const_1 )//_sage_const_2 ] = _sage_const_1 
            relabel[x] = (tuple(t),k+_sage_const_1 )
    return P.relabel(relabel)

def GT_Tri_slicing(n):
    G = Poset((Bigrassmannians(n,'GTtri'),lambda p,q:p<=q)).hasse_diagram()
    S = [[] for _ in range(n-_sage_const_1 )]
    for x in G:
        S[x.main[_sage_const_2 ]-x.main[_sage_const_0 ]-_sage_const_2 ].append(x)
    P = Poset(Slicing_to_chains(G.transitive_closure(),S))
    relabel = {}
    ranks,f = [[] for _ in range(n-_sage_const_1 )],P.rank_function()
    for x in P:
        ranks[f(x)].append(x)
    for k in range(_sage_const_1 ,n):
        for x in ranks[k-_sage_const_1 ]:
            s = k*[_sage_const_0 ]
            for i in x[_sage_const_0 ]:
                a,b,c = i.main
                s[c-b-_sage_const_1 ] += _sage_const_1 
            s2 = (n-k)*[_sage_const_0 ]
            for i in range(k):
                s2 = s2[:s[i]]+[_sage_const_1 ]+s2[s[i]:]
            relabel[x] = tuple(s2)
    return P.relabel(relabel)
...
def CanonicalJoinComplex(L):
    ...
    for x in L:
        c = L.lower_covers(x)
        if len(c) == _sage_const_1 :
            V.append(x)
        elif len(c) == _sage_const_2 :
...
def Spine(L):
    ...
    """
    h = L.height() - _sage_const_1 
    x = L.minimal_elements()[_sage_const_0 ]
    D = {x: _sage_const_0 }
    l = [x]
    for k in range(h):
        l2 = set()
        for x in l:
            for y in L.upper_covers(x):
                D[y] = max(D.get(y, _sage_const_0 ), k + _sage_const_1 )
                l2.add(y)
        l = l2
    s = list(l)
    for k in range(h):
...
def SpineIrreducibles(L):
    ...
    return P.relabel(lambda x: Edge_to_JoinIrr(L, S.lower_covers(x)[_sage_const_0 ], x))
...
def BiclosedSets(G):
    ...
    BS = []
    for S in Subsets(G):
        if all(((t[_sage_const_0 ] in S) != (t[_sage_const_1 ] in S)) or (t[_sage_const_0 ] in S) == (C[t] in S) for t in C):
            BS.append(S)
    return Poset((BS,lambda p,q:p.issubset(q)))

L = LatticePoset([[_sage_const_1 , _sage_const_2 , _sage_const_3 , _sage_const_4 ], [[_sage_const_1 , _sage_const_2 ], [_sage_const_1 , _sage_const_3 ], [_sage_const_2 , _sage_const_4 ], [_sage_const_3 , _sage_const_4 ]]], cover_relations=true)
M = MOPLattice(L)
